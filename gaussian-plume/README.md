...
在单点能力之上另长了**多点源叠加**一层：一批高矮不一、位置不同的烟囱共用一次
风向/风速/稳定度，按统一平面坐标投影后逐源套同一套烟羽公式，在每个受体点上
线性叠加，并拆出每根烟囱的贡献与占比。

只做稳态这一件事。天气预报展示、城市空气质量看板、标准大气温度层结查表、
风向/复杂地形流场、非稳态扩散、等值线成图**不在范围内**。
...
### 1.5 多点源叠加（新增的一层，不改任何单点数学）

单批作业统一给定一次**风向角、风速、稳定度**；每根烟囱带自己的水平位置、
源强、物理源高（热抬升可选）。对每个「源→受体」：

1. **坐标投影（新骨架，`internal/projection`）**：源与受体先摆进共享平面
   坐标系（x 东、y 北，m），按下风方向把位移投影成沿风分量与侧风分量：

   ```
   x = (Rx−Sx)·sin θ + (Ry−Sy)·cos θ     # 沿风下风分量（>0 受体在源下风向）
   y = (Rx−Sx)·cos θ − (Ry−Sy)·sin θ     # 垂直风向侧风分量（浓度只依赖 y²）
   θ = wind_dir + 180°
   ```

2. **风向角约定全服务钉死一种**（气象罗盘）：`wind_dir` 为风的**来向**，
   **自正北起算、顺时针、单位度**（0=北风、90=东风、180=南风、270=西风）。
   归一化到 [0,360)，所有源/受体统一套用；不允许另一处代码再按数学角
   （x 轴起算逆时针）另算。

3. **原样复用单点公式**：投影出的 (x, y) 直接喂给现有 plume/dispersion/rise，
   公式、扩散参数、热抬升一个字不改；受体高度固定 z=0（地面）。

4. **线性叠加**：同一受体点上各源贡献直接相加；稳态高斯解对源强线性，故
   各源单独算的贡献之和恰好等于整批合提交的合成浓度。

**不做方位特判剔除源**：受体落在源上风（x<0）或正侧风（x=0）时，烟羽稳态
解只定义在下风半平面，贡献按自然极限取 **0**（该源仍逐项出现在贡献清单里，
不带 error）。批量里某个源**排放条件非法**（源强为负、物理源高为负、抬升参数
非法等）则按**零贡献**处理并在该源的 `error` 字段写明原因，其余源照算，
作业 `status:"partial"`，绝不一报错整批废掉。

被自动化测试锁死的多源判据（`internal/multisource/`、`internal/projection/`、
`internal/multijob/`、`internal/api/handlers_test.go`、`internal/store` 集成测试）：

- **单源退化精确一致**：单源且在受体上风轴线上，多源结果与单点公式逐位吻合
  （含带侧风偏移的情形）；
- **双源线性叠加**：任意位置/源高/稳定度/风向组合，单独贡献之和 == 合成浓度，
  且批量内逐项贡献 == 各自单独提交的结果，占比之和为 1；
- **上风/侧风压低**：同源挪到正上风、正侧风贡献为 0，显著小于正下风轴线上；
- **单坏源隔离**：负源强/负源高/坏抬升只零掉自己并说明原因，不拖垮整批。
...
internal/projection     共享平面坐标 → 沿风下风/侧风分量投影（多源新骨架）
internal/multisource    多源逐源贡献、线性累加、占比拆解、逐源容错（叠一层）
internal/multijob       多源作业编排、缺省标识、入库与回查（与 job 平行）
internal/validate       结构化输入校验（作业级 / 点位级）
internal/job            单源扫描编排、逐点计算、有效源高解析、作业入库与回查、示范作业
internal/store          PostgreSQL 16 持久化（pgx/v5，启动幂等迁移；含多源主表+源明细表）
internal/api            Gin HTTP 接口（单源三件 + 多源两件）
internal/config         环境变量配置
migrations/             SQL 迁移（store 内嵌同一份，启动自动应用）
```

旧的单点计算（`/calculate`）与单源扫描作业（`/scans`、`/scans/:id`）原样保留、
行为不变；`projection → multisource → multijob` 是在它们之上新长出来的一层，
不替换旧流程。
...
### 3.4 多点源叠加：提交作业 — `POST /api/v1/multi-source-jobs`

一批点源（各自水平坐标、源强、物理源高，热抬升可选）+ 一组受体点，共用一次
`wind_dir`（气象罗盘来向角，正北起算顺时针，度）、`u`、`stability`。
逐受体点返回合成浓度 `c` 与每根烟囱的贡献 `c`、`share` 占比、投影后的
`downwind_x/crosswind_y`、所用 σ/有效源高；落库并返回 `job_id`。

```bash
curl -s localhost:8080/api/v1/multi-source-jobs -H 'Content-Type: application/json' -d '{
  "wind_dir": 0, "u": 5, "stability": "D",
  "receptors": [
    {"id": "厂界东", "x": 0, "y": -1000},
    {"id": "最近居民点", "x": 400, "y": -800}
  ],
  "sources": [
    {"id": "1#烟囱", "x": 0,   "y": 0, "q": 0.10, "physical_h": 50},
    {"id": "2#烟囱", "x": 150, "y": 0, "q": 0.05, "physical_h": 80,
     "buoyancy_rise": {"vs": 20, "ds": 3, "ts": 400, "ta": 283}},
    {"id": "3#烟囱", "x": -120, "y": 0, "q": 0.02, "use_given_h": true, "effective_h": 35}
  ]
}'
```
...
```json
{
  "job_id": "…", "status": "ok", "wind_dir": 0, "u": 5, "stability": "D",
  "receptors": [{
    "receptor_id": "厂界东", "index": 0, "x": 0, "y": -1000,
    "c": 5.5e-7,
    "contributions": [
      {"source_id": "1#烟囱", "index": 0, "c": 5.5e-7, "share": 1.0,
       "downwind_x": 1000, "crosswind_y": 0,
       "sigma_y": 70.0, "sigma_z": 26.0, "effective_h": 50},
      {"source_id": "2#烟囱", "index": 1, "c": 0.0, "share": 0.0,
       "downwind_x": 1000, "crosswind_y": 150, "sigma_y": 70.0, "sigma_z": 26.0,
       "effective_h": 223.0, "delta_h": 143.0, "rise_regime": "neutral_buoyant_strong"}
    ]
  }]
}
```
...
- 受体在某源**上风/正侧风**（投影 `downwind_x ≤ 0`）时该源贡献自然为 `0`、
  `share` 为 `0`，**不带 error**，源仍逐项列出；
- 某源**排放条件非法**（`q<0`、`physical_h<0`、抬升参数非法、坐标非有限等）：
  该源 `c=0` 且 `error` 说明原因，其余源照算，作业仍 `201`、`status:"partial"`；
- 作业级公共条件非法（`u≤0`、稳定度非法、`wind_dir` 非有限、源/受体列表为空、
  受体坐标非有限）整单 `400`，不落库；
- 源/受体不填 `id` 时自动补 `source-0`/`receptor-0` 这类序号标识。

### 3.5 多点源叠加：按标识回查 — `GET /api/v1/multi-source-jobs/:id`

返回完整请求（**每个源各自的原始输入**，含坐标/源强/源高/抬升参数）与逐受体、
逐源贡献结果；不存在返回 `404`。每个源另逐行持久化在
`multi_source_job_sources` 明细表，回查的不是一个总浓度数字。
...
坏点隔离、并发不串写、示范作业幂等与手算值；以及多源四条钉死判据——**单源退化与
老公式完全一致、双源线性叠加（任意位置/源高/稳定度）、上风/正侧风贡献压到 0、
批量中单个非法源只零掉自己不拖垮整批**，外加坐标投影的罗盘约定/保距/等价角度、
多源作业并发不串写与 PostgreSQL 主表+源明细表的完整回查。
